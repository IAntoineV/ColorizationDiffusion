...
class DiffusionConditioned:
    ...
    def backward_diffusion(self,conditioning, xt, t, sampling=False):
        """
        ...
        t = t.view(-1,1,1,1)
        epsilon_theta = self.model(conditioning,xt, t)
    ...
    @torch.no_grad()
    def sampling(self, conditioning,shape=None, xT=None,T=None, to_log=False):
        """
        ...
        T_batched = T* torch.ones((conditioning.shape[0],), device=self.device, dtype=torch.int)
        logs={}
        for denoising_step in range(T):
            timestep = T_batched - denoising_step -1
            xt,_ = self.backward_diffusion(conditioning,xt, timestep, sampling=True)
            #xt = xt.clip(0,1)
            if to_log:
                logs[int(timestep)] = xt
        x0 = xt
        return x0, logs

    def train(self, dataloader, lr=1e-4, num_epochs=10, verbose=True, to_save=False):
        ...
        for epoch in range(num_epochs):
            i=0
            loss_avg = 0
            for k,(conditionnings,images) in enumerate(dataloader):
                i+=1
                images = images.to(device)
                conditionnings = conditionnings.to(device)
                ...
                _, noise_pred = self.backward_diffusion(conditionnings, x_t, t)
                ...
                loss.backward()
                optimizer.step()
                loss_avg = (k*loss_avg + loss.item())/(k+1)
                dataloader.log("loss", loss_avg)
            if verbose:
                print(f"Epoch {epoch + 1}/{num_epochs}, Loss: {loss_avg:.4g}, nb_step: {i}")
            if to_save:
